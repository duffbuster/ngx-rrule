...
const TSC_ARGS = (config = 'build'): Array<any> => [
  '-p',
  `${process.cwd()}/src/lib/tsconfig-${config}.json`
];

/**
 * @description Create an Observable of a spawned child process.
 */
function spawnObservable(command, args): void {
...
function generateBundle(input, file, globals, name, format): any {
...
function createUmd(globals): any {
  const name = 'ngx-rrule';
  const entry = `${process.cwd()}/dist/es5/index.js`;

  return generateBundle(
    entry,
    `${process.cwd()}/dist/packages-dist/rrule.umd.js`,
    globals,
    name,
    'umd'
  );
...
function createEs(globals, target): any {
  const name = 'ngx-rrule';
  const entry = `${process.cwd()}/dist/${target}/index.js`;

  return generateBundle(
    entry,
    `${process.cwd()}/dist/packages-dist/rrule.${target}.js`,
    globals,
    name,
    'es'
  );
...
function buildModule(globals): any {
  const es2015$ = spawnObservable(NGC, TSC_ARGS()) as any;
  const esm$ = spawnObservable(NGC, TSC_ARGS('esm')) as any;

  return Observable.forkJoin(es2015$, esm$);
}

function createBundles(globals): any {
...
function copyFiles(): any {
  ...
  return Observable.forkJoin(
    ...
    copyAll(
      `${process.cwd()}/dist/es2015/**/*.metadata.json`,
      `${process.cwd()}/dist/packages-dist`
    ),
    Observable.from(
      copy(
    ...
    Observable.from(
      copy(
        `${process.cwd()}/src/lib/rrule.css`,
        `${process.cwd()}/dist/packages-dist/rrule.css`
      )
    )
  );
}

function buildLibrary(globals): any {
